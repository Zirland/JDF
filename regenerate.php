...
$query9 = "SELECT route_id FROM route WHERE active=1;";
	...
	while ($row9 = mysqli_fetch_row($result9)) {
		...
		if ($result55 = mysqli_query($link, $query55)) {
			while ($row55 = mysqli_fetch_row($result55)) {
				...
				$now = microtime(true);
				$dlouho = $now-$prevnow;
				$current = "$trip_id headsign: $dlouho\n";
				$prevnow = $now;
				file_put_contents($file, $current, FILE_APPEND);
				$current = "";
