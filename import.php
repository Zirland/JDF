	...
	case 'import' :
		...
		if ($result117 = mysqli_query($link, $query117)) {
			while ($row117 = mysqli_fetch_row($result117)) {
				...
				$vznik = $logid;
				if ($logid > 999999) {$vznik = substr($vznik, -6);}
				if ($logid < 100000) {$vznik = "0".$vznik;}
				if ($logid < 10000) {$vznik = "0".$vznik;}
				if ($logid < 1000) {$vznik = "0".$vznik;}
				if ($logid < 100) {$vznik = "0".$vznik;}
				if ($logid < 10) {$vznik = "0".$vznik;}
				...
				if (substr($weekmatrix, 0, 1) == 1) { // pondělí
					$dy = 1;
					for ($wk = 0; $wk < 60; $wk++) {
						$index = $shift + $dy + ($wk * 7);
						$matrix[$index] = 1;
				...
				if (substr($weekmatrix, 1, 1) == 1) { // úterý
					$dy = 2;
					for ($wk = 0; $wk < 60; $wk++) {
						$index = $shift + $dy + ($wk * 7);
						$matrix[$index] = 1;
				...
				if (substr($weekmatrix, 2, 1) == 1) { // středa
					$dy = 3;
					for ($wk = 0; $wk < 60; $wk++) {
						$index = $shift + $dy + ($wk * 7);
						$matrix[$index] = 1;
				...
				if (substr($weekmatrix, 3, 1) == 1) { // čtvrtek
					$dy = 4;
					for ($wk = 0; $wk < 60; $wk++) {
						$index = $shift + $dy + ($wk * 7);
						$matrix[$index] = 1;
				...
				if (substr($weekmatrix, 4, 1) == 1) { // pátek
					$dy = 5;
					for ($wk = 0; $wk < 60; $wk++) {
						$index = $shift + $dy + ($wk * 7);
						$matrix[$index] = 1;
				...
				if (substr($weekmatrix, 5, 1) == 1) { // sobota
					$dy = 6;
					for ($wk = 0; $wk < 60; $wk++) {
						$index = $shift + $dy + ($wk * 7);
						$matrix[$index] = 1;
				...
				if (substr($weekmatrix, 6, 1) == 1) { // neděle a svátky
					$dy = 0;
					for ($wk = 0; $wk < 60; $wk++) {
						$index = $shift + $dy + ($wk * 7);
						$matrix[$index] = 1;
