...
$akt_trip = "SELECT route_id,trip_id,trip_headsign,direction_id,shape_id,wheelchair_accessible,bikes_allowed FROM trip WHERE active = '1' AND route_id NOT IN (SELECT route FROM `ignore`) AND trip_id > " . $l1 . "00000000 AND trip_id < " . $l2 . "00000000 AND trip_id IN (SELECT trip_id FROM jizdy WHERE datum>='$dnes' AND datum<'$tyden');";
...
$tripstops = "SELECT trip_id,arrival_time,departure_time,stop_id,stop_sequence,pickup_type,drop_off_type FROM stoptime WHERE trip_id IN (SELECT trip_id FROM trip WHERE active = '1' AND route_id NOT IN (SELECT route FROM `ignore`) AND trip_id > " . $l1 . "00000000 AND trip_id < " . $l2 . "00000000 AND trip_id IN (SELECT trip_id FROM jizdy WHERE datum>='$dnes' AND datum<'$tyden'));";
